...
class CarDevice extends Homey.Device {
      ...
      setTimeoutPromise(60 * 1000)
        .then(() => {
          if (!this.client.controller || !this.client.controller.session || !this.client.controller.session.tokenExpiresAt) {
            ...
            this.restartDevice(1 * 1000).catch((error) => this.error(error));
          }
        })
  ...
  async doPoll(forceOnce) {
    ...
    try {
      ...
      // update ABRP
      if (this.isEV && refresh) this.abrpTelemetry(info).catch((error) => this.error(error));

      // update capabilities and flows
      this.handleInfo(info).catch((error) => this.error(error));
  ...
  async onSettings() { // { newSettings }) {
    ...
    this.migrated = false;
    this.restartDevice(250).catch((error) => this.error(error));
  ...
  async registerListeners() {
    try {
      ...
      this.registerMultipleCapabilityListener(['charge_target_slow', 'charge_target_fast'], async (values) => {
        ...
        this.setChargeTargets(targets, 'app').catch((error) => this.error(error));
      }, 10000);
